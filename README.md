...
src/main/scala/evm/
  ...
  value/   primitive value types
    ...
    Precompiles.scala          The precompiled contracts: identity (verified) and MODEXP (verified pow-mod spec + fast path), with ecRecover, SHA-256, RIPEMD-160, BLAKE2F, bn254 ecAdd/ecMul and P-256 verify as trusted @extern reference implementations, each validated against official vectors (ecPairing and KZG, which need pairings, are the remaining two)
